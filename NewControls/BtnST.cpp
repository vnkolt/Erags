...
IMPLEMENT_DYNCREATE(CButtonST, CButton)
...
{
	if (bCheckForNULL && FALSE==m_fResourceShared)
	{
		// Destroy icons
...
void CButtonST::OnEnable(BOOL bEnable) 
{
	...
	if (bEnable == FALSE)	
	{
/************************************** : by VK 16.12.2004 at 11:41:11 **\
		CWnd*	pWnd = GetParent()->GetNextDlgTabItem(this);
		if (pWnd)
			pWnd->SetFocus();
		else
			GetParent()->SetFocus();
\************************************** : by VK 16.12.2004 at 11:41:14 **/
		CancelHover();
	} // if
} // End of OnEnable
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	pDC->DrawState(	rImage.TopLeft(),
					...
					(CBrush*)NULL);
/************************************** : by VK 23.12.2004 at 15:21:53 **\
  ::DrawIconEx(pDC->m_hDC, rpItem->left, rpItem->top, m_csIcons[byIndex].hIcon, m_csIcons[byIndex].dwWidth, m_csIcons[byIndex].dwHeight, 0, (HBRUSH)GetMenuBarBrush()->m_hObject, DI_NORMAL);
\************************************** : by VK 23.12.2004 at 15:21:56 **/
} // End of DrawTheIcon
...
void CButtonST::SetGradientParent(BOOL fGradientParent, BOOL bRepaint/* = FALSE*/) {
  m_bGradientParent = fGradientParent;
	// Repaint the button
	if (bRepaint) Invalidate();
} // End of DrawTransparent
