...
{
  int result = DefWindowProc ( TB_REPLACEBITMAP, wParam, lParam);
  if(result)
  {
    ...
    if(pReplaceBitmap)
    {
      ...
      int nIndex = DefWindowProc ( TB_ADDBITMAP, pReplaceBitmap->nButtons, (LPARAM)&addBitmap);
      if(nIndex!=-1)
      {
...
{
  int result = DefWindowProc ( TB_ADDBITMAP, nButtons, lParam);
  if(result!=-1)
  {
      ...
      if(pAddImage)
      {
        ...
        if(hBitmap)
        {
          ...
          if(!m_ImageList.GetSafeHandle())
          {
            ...
            m_ImageList.Create(myInfo.bmWidth/nButtons,myInfo.bmHeight,ILC_COLORDDB|ILC_MASK,0,10);
          }
          ...
          if(!m_ImageListDisabled.GetSafeHandle())
          {
            ...
            m_ImageListDisabled.Create(myInfo.bmWidth/nButtons,myInfo.bmHeight,ILC_COLORDDB|ILC_MASK,0,10);
          }
