...
void CSmtp::Send()
{
	...
	if(hSocket==NULL)
	{
		if(!ConnectRemoteServer(m_sSMTPSrvName.c_str(), m_iSMTPSrvPort))
...
bool CSmtp::ConnectRemoteServer(const char *szServer, const unsigned short nPort_)
{
	...
	try
	{
		...
		if(IsKeywordSupported(RecvBuf, "AUTH") == true)
		{
			if(!m_sLogin.size())
			...
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				...
				delete[] ustrLogin;
				sprintf(SendBuf, "AUTH PLAIN %s", encoded_login);
				SendData(pEntry);
				ReceiveResponse(pEntry);
			}
...
void CSmtp::FormatHeader(char* header)
{
    ...
    /*
		if(m_bHTML) strcat(header,"Content-Type: text/html; charset=\"US-ASCII\"\r\n");
		else strcat(header,"Content-type: text/plain; charset=\"KOI8-R\"\r\n");
...
const char* CSmtp::GetLocalHostName()
{
	...
	m_sLocalHostName = str;
	delete[] str;
	if(m_sLocalHostName.empty())
		return NULL;
...
const char* CSmtp::GetMsgLineText(unsigned int Line) const
{
  ...
  // std::string encoded_s = base64_encode(reinterpret_cast<const unsigned char*>(MsgBody.at(Line).c_str()),MsgBody.at(Line).size());
  // return encoded_s.c_str();
		...
		if(FD_ISSET(hSocket,&fdread) || (read_blocked_on_write && FD_ISSET(hSocket,&fdwrite)) )
		{
			while(1)
			{
				...
				if(ssl_err == SSL_ERROR_NONE)
				{
					...
					memcpy(RecvBuf, buff+offset, res);
					offset += res;
					if(SSL_pending(ssl))
					{
						continue;
