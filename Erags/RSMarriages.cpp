...
void CRSMarriages::DoFieldExchange(CDaoFieldExchange* pFX)
{
  try {
	...
	DFX_Byte(pFX, _T("[MFAMILY_STATUS]"), m_MFAMILY_STATUS);
	DFX_Short(pFX, _T("[MACT_NUMBER]"), m_MACT_NUMBER);
	DFX_DateTime(pFX, _T("[MACT_DATE]"), m_MACT_DATE);
	DFX_Text(pFX, _T("[MACT_BY]"), m_MACT_BY);
	...
	DFX_Byte(pFX, _T("[WFAMILY_STATUS]"), m_WFAMILY_STATUS);
	DFX_Short(pFX, _T("[WACT_NUMBER]"), m_WACT_NUMBER);
	DFX_DateTime(pFX, _T("[WACT_DATE]"), m_WACT_DATE);
	DFX_Text(pFX, _T("[WACT_BY]"), m_WACT_BY);
