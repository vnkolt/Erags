...
BOOL CNewWizDialog::ActivatePage(CNewWizPage* pPage)
{
  ...
  ScreenToClient(&rect);
  pPage->SetWindowPos(NULL, rect.left+1, rect.top+1, 0, 0, 
...
LPCSTR CNewWizDialog::GetHelpPostfix() {
  return (IDC_RADIO_UKR==theApp.m_HelpLang) ? _T("u") : _T("r");
}
void CNewWizDialog::OnHelp() {
  ...
  if ( g_hHHCTRLInstance ) {
    ...
    if ( htmlHelp ) {
      ...
      szHelpDirectory.Format(_T("%s\\Help"), theApp.m_szUnzipFolder);
      szHelpPostfix = GetHelpPostfix();

      CString szFileName;
      szFileName.Format(_T("%s\\Erags_%s.chm::/Setup_%s.htm"), szHelpDirectory, szHelpPostfix, szHelpPostfix);
...
BOOL CNewWizDialog::OnEraseBkgnd(CDC* pDC) 
{
	// TODO: Add your message handler code here and/or call default
  CRect r;
  GetClientRect(&r);
	pDC->FillSolidRect(&r, RGB(255,255,255));
	//return CNewWizPage::OnEraseBkgnd(pDC);
  return TRUE;
}
