...
*/
  ...
  img.CreateTC(IDB_HOTTOOLBAR, 22, 20, RGB(255, 0, 255));
	m_wndToolBar.GetToolBarCtrl().SetHotImageList(&img);
	img.Detach();
	// img.Create(IDB_COLDTOOLBAR, 22, 0, RGB(255, 0, 255));
  img.CreateTC(IDB_COLDTOOLBAR, 22, 20, RGB(255, 0, 255));
  ...
  // create a combo box for the address bar
	...
	m_wndAnimate.Create(WS_CHILD | WS_VISIBLE|ACS_TRANSPARENT, CRect(0, 0, 10, 10), this, AFX_IDW_TOOLBAR + 2);
  m_wndAnimate.Open(IDR_FRONTPAGE);
...
// set up Favorites menu
  ...
  //strcpy(szPath, GetModulePath());
  sprintf(szPath, "%s\\Laws", GetModulePath());
...
{
	((CULawsView*)GetActiveView())->Navigate2(m_astrFavoriteURLs[nID-0xe00], 0, NULL);
}
...
void CMainFrame::OnNewAddress()
{
	...
	((CULawsView*)GetActiveView())->Navigate2(str, 0, NULL);
}
...
BOOL CMainFrame::DestroyWindow() {
  ...
  if ( m_FoundBar.GetSafeHwnd() ) {
    CString sProfile(_T("FoundBar"));
	  m_FoundBar.SaveState(sProfile);
...
void CMainFrame::OnSearch() {
  ...
  if ( theSearhBar.GetSafeHwnd() ) {
    if ( theSearhBar.IsWindowVisible() ) {
      ShowControlBar(&theSearhBar, false, false); // Hide
      // ShowFoundBar(false);
    }
    else {
      ShowControlBar(&theSearhBar, true, false); // Show
      // ShowFoundBar(true);
    }
  }
  else {
    CToolBarCtrl& ToolBarCtrl = m_wndToolBar.GetToolBarCtrl();
    UINT iSearch = ToolBarCtrl.CommandToIndex(ID_SEARCH);
    CRect r;
    m_wndToolBar.GetItemRect(iSearch, &r);
    m_wndToolBar.ClientToScreen(&r);
    theSearhBar.Create();
    EnableDocking(CBRS_ALIGN_ANY);
    theSearhBar.EnableDocking(0);
    FloatControlBar(&theSearhBar, CPoint(r.left, r.top+r.Height()));
    ShowControlBar(&theSearhBar,true,false);
    // ShowFoundBar(true);
  }
}

void CMainFrame::ShowFoundBar(BOOL fShow) {
  if ( m_FoundBar.GetSafeHwnd() ) {
    if ( m_FoundBar.IsWindowVisible() && false==fShow) {
      SaveBarState(_T("FoundBar"));
      ShowControlBar(&m_FoundBar, false, false); // Hide
    }
    else {
      ShowControlBar(&m_FoundBar, true, false); // Show
    }
    RecalcLayout();
    return;
  }

  if (!m_FoundBar.Create(_T("Знайдені документи"), this, CSize(600, 200), true, FOUND_BAR_ID)) {
    TRACE0("Failed to create mybar\n");
    return;
  }

  m_FoundBar.SetBarStyle(m_FoundBar.GetBarStyle() | CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC);
  EnableDocking(CBRS_ALIGN_ANY);
  m_FoundBar.EnableDocking(CBRS_ALIGN_ANY);
  DockControlBar(&m_FoundBar, AFX_IDW_DOCKBAR_BOTTOM);

  CString sProfile(_T("FoundBar"));
  if (VerifyBarState(sProfile)) {
    m_FoundBar.LoadState(sProfile);
