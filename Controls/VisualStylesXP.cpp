...
int     CVisualStylesXP::m_nUseCount = 0;
...
///////////////////////////////////////////////////////////////////////////////
// DrawThemeText
BOOL CVisualStylesXP::DrawThemeText(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, LPCTSTR lpszText, DWORD dwTextFlags, DWORD dwTextFlags2, const RECT *pRect) {
  //TRACE(_T("in CXThemeHelper::DrawThemeText\n"));

  BOOL ok = FALSE;

  if (m_hThemeDll && hTheme)
  {
    HRESULT hr = S_OK;

#ifdef _UNICODE

    hr = DrawThemeText(hTheme, hdc, iPartId, iStateId, lpszText, wcslen(lpszText), dwTextFlags, dwTextFlags2, pRect);
    ...
    if (pszWide)
    {
      ...
      hr = DrawThemeText(hTheme, hdc, iPartId, iStateId, pszWide, wcslen(pszWide), dwTextFlags, dwTextFlags2, pRect);
      delete pszWide;
    }
