...
void CFlatHeaderCtrl::DrawCtrl(CDC* pDC)
{
	...
	for(INT i=0;i<iItems;i++)
	{
		...
		HDITEM hditem;
		hditem.mask = HDI_WIDTH|HDI_FORMAT|HDI_LPARAM|HDI_TEXT|HDI_IMAGE|HDI_BITMAP;
...
INT CFlatHeaderCtrl::DrawText(CDC* pDC, CRect rect, LPHDITEM lphdi)
{
  ...
  // by VK at 10.02.2005 1:18:04
  COLORREF clr = pDC->GetTextColor();
  if ( lphdi->lParam )
    pDC->SetTextColor(lphdi->lParam);
  else
    pDC->SetTextColor(m_crText);
