...
#ifndef HDF_SORTUP                      // by VK
#define HDF_SORTUP              0x0400  // defined in comctrl.h if  _WIN32_WINNT >= 0x501
#endif

#ifndef HDF_SORTDOWN                    // by VK
#define HDF_SORTDOWN            0x0200  // defined in comctrl.h if  _WIN32_WINNT >= 0x501
#endif
...
{ 	// Must free if it's not alreay do
	//
	INT_PTR nCols = columns.GetSize();
	for(INT_PTR nCol = 0; nCol < nCols;nCol++)
...
void CListCtrlStyled::SetSortedColumn(int nColumn, BOOL fAscending) { // by VK
  ...
  for( int i(0); i < nColumns; i++ ) {
    ...
    hdItem.fmt &= ~(HDF_SORTUP|HDF_SORTDOWN);               // clear up/down arrow for all items
    if ( i==nColumn ) {
      hdItem.fmt |= fAscending ? HDF_SORTUP : HDF_SORTDOWN; // set for up/down arrow for specefied column
    }
...
void CListCtrlStyled::Free_LS_item(LS_item* lpLS_item)
{
	...
	{	// We must delete the subitems structure also
		//
		INT_PTR nSubItems = lpLS_item->subitems.GetSize();

		for(INT_PTR nSubItem = 0; nSubItem < nSubItems; nSubItem++)
