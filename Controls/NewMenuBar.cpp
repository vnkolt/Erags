...
{ 
  ...
  case CNewMenu::STYLE_MONOCHROME:
  case CNewMenu::STYLE_MONOCHROME_NOBORDER:
    {
      COLORREF colorMid    = /*(g_clrLeft+g_clrRight+1)/2*/GetSysColor(COLOR_HIGHLIGHT);
      COLORREF colorSel    = LightenColor(60,g_clrLeft);
      COLORREF colorBorder = DarkenColor(60,g_clrLeft);
      ...
      default:
        {
          CRect temp = rect;
          temp.left+=2;
          CControlBar::DrawGripper(pDC,temp);
        }
        break;
      }
    }
...
BOOL CNewMenuBar::OnEraseBkgnd(CDC* pDC)
{
  ...
  ScreenToClient(&r);
  if ( g_fUseGradient ) {
    GradientEraseBkgnd(this, GetParentFrame(), pDC, r);
  }
  else {
    pDC->FillSolidRect(&r, g_clrLeft);
  }
  return true;
}
...
void CNewMenuBar::OnTimer(UINT nIDEvent)
{
  ...
  BOOL bFocus = pFrame ? IsFocusChild(pFrame) : FALSE;
  if(!bFocus)
  {
    SetSel(-1,TR_NONE);
    EnableTimer(FALSE);
