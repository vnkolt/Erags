...
BOOL CListEditCtrl::PreTranslateMessage(MSG* pMsg)
{
    ...
    LPARAM lParam = pMsg->lParam;
    ...
    else if ( VK_UP==pMsg->wParam || VK_DOWN==pMsg->wParam ) {
      pMsg->wParam = VK_RETURN;
      ::TranslateMessage(pMsg);
			::DispatchMessage(pMsg);
      GetParent()->PostMessage(WM_KEYDOWN, wParam,    lParam);
      GetParent()->PostMessage(WM_KEYDOWN, VK_RETURN, lParam);
			return 1;
    }
      ...
      CString s;
      GetWindowText(s);
      int nLenght = s.GetLength();
      int nStartChar, nEndChar;
      GetSel(nStartChar, nEndChar);
      if ( nLenght==nStartChar && nLenght==nEndChar ) {
        pMsg->wParam = VK_RETURN;
        ::TranslateMessage(pMsg);
	  		::DispatchMessage(pMsg);
        SendKeys(VK_RIGHT, 0);
