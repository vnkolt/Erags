...
LRESULT CPPToolTip::SendNotify(LPPOINT pt, PPTT_INFO & ti) 
{ 
...
void CPPToolTip::SetNewTooltip(HWND hWnd, const PPTT_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = PPTOOLTIP_NORMAL */)
{
  if ( hWnd ) { // by VK
...
/************************************** : by VK at 16.05.2004 22:08:36 **\
  CRect rcControl;
  CWnd* pCtrlWnd = WindowFromPoint(*lpPoint);
  if ( pCtrlWnd ) {
    CRect rCtrl;
    pCtrlWnd->GetWindowRect(&rCtrl);
    ptAnchor.x = lpPoint->x;
    ptAnchor.y = lpPoint->y;
  }
\************************************** : by VK at 16.05.2004 22:08:41 **/
  ...
  // ************************************** : by VK at 16.05.2004 21:07:22
  /************************************** : by VK at 18.05.2004 23:37:33 **\
  CRect rParent;
  GetParent()->GetWindowRect(&rParent);
  while( rect.left-m_szOffsetShadow.cx < rParent.left ) {
    rect.OffsetRect(1, 0);
    ptAnchor.x--;
  }
  while( rect.right > rParent.right ) {
    rect.OffsetRect(-1, 0);
    ptAnchor.x++;
  }
  \************************************** : by VK at 18.05.2004 23:37:37 **/
  // ************************************** : by VK at 16.05.2004 21:07:24
...
HWND CPPToolTip::FindTool(const LPPOINT lpPoint, PPTT_INFO & ti)
{
...
BOOL CPPToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTT_INFO & ti)
{
...
HWND CPPToolTip::FindToolBarItem(POINT point, PPTT_INFO & ti)
{
...
//		ti				- Reference to PPTT_INFO structure containing the parameters of the tooltip 
//
// Remarks:
...
void CPPToolTip::AddTool(CWnd * pWnd, PPTT_INFO & ti)
{
...
void CPPToolTip::ShowHelpTooltip(LPPOINT pt, PPTT_INFO & ti)
{
