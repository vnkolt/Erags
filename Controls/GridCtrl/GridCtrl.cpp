...
void CGridCtrl::SetSelectedRange(const CCellRange& Range, BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/) {
  SetSelectedRange(Range.GetMinRow(), Range.GetMinCol(), Range.GetMaxRow(), Range.GetMaxCol(), bForceRepaint, bSelectCells);
}
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
    ...
    if( pItem->mask & GVIF_STRMASK)
      pCell->SetStrMask(pItem->strMask);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
    ...
    if( pItem->mask & GVIF_STRMASK)
      pItem->strMask = pCell->GetStrMask();
...
void CGridCtrl::SetDBMode(BOOL bDBMode) {
  m_bDBMode = bDBMode;
}
