...
CString CGridCell::GetStrMask() const {
  return m_strMask;
}
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar) {
  if ( m_bEditing ) {      
    ...
    if ( m_strMask==_T("00.00.0000") ) {
      m_pEditWnd = new CInPlaceDateEdit(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    else
...
void CGridCell::EndEdit() {
  if (m_pEditWnd) {
    if ( m_strMask==_T("00.00.0000") )
      ((CInPlaceDateEdit*)m_pEditWnd)->EndEdit();
    else
