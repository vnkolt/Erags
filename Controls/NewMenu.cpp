...
COLORREF  g_clrLeft  = GetSysColor(COLOR_MENU);
COLORREF  g_clrRight = GetSysColor(COLOR_WINDOW);
BOOL      g_fUseGradient = TRUE;
...
void MenuDrawText(HDC hDC ,LPCTSTR lpString,int nCount,LPRECT lpRect,UINT uFormat)
{
  ...
  int buf_size = nCount*sizeof(TCHAR)+2;
  TCHAR* pBuffer = (TCHAR*)_alloca(buf_size);
#ifdef _STRCPY_S
  _tcsncpy_s(pBuffer, buf_size, lpString, nCount);
#else
...
CBitmap g_GradientBitmap;
int     g_GradientBitmapWidth  = 800;
int     g_GradientBitmapHeight = 20;
void CreateBackgroundBitmap() {
  if ( g_GradientBitmap.m_hObject )
    g_GradientBitmap.DeleteObject();
  ...
  CRect rect(0,0,GetSystemMetrics(SM_CXFULLSCREEN)+16,20);

  g_GradientBitmapWidth  = rect.Width();
  g_GradientBitmapHeight = rect.Height();

  g_GradientBitmap.CreateCompatibleBitmap(pDcDsk,rect.Width(),rect.Height());
  CBitmap* pOldBitmap = clientDC.SelectObject(&g_GradientBitmap);
  DrawGradient(&clientDC, rect, g_clrLeft, g_clrRight, TRUE, TRUE);
...
/************************************** : by VK at 18.11.2005 0:47:27 **\
    ...
    if (g_Shell > Win98 && 
        (menuDrawMode==CNewMenu::STYLE_XP_2003 || menuDrawMode==CNewMenu::STYLE_COLORFUL || menuDrawMode==CNewMenu::STYLE_MONOCHROME))
    {
      CreatePatternBrush(&g_GradientBitmap);
    }
    else
    {
      CreateSolidBrush(barColor);
    }
\************************************** : by VK at 18.11.2005 0:47:31 **/
    CreateSolidBrush(barColor);
  }
};
...
// MyStretchBlt prevent this bug.
//
// MSDN Article ID	:	269585
// http://support.microsoft.com/kb/269585
//
/************************************** : by VK at 06.01.2007 11:35:43 **\
BOOL MyStretchBlt(CDC* pDC, int x, int y, int nWidth, int nHeight, CDC* pSrcDC, int xSrc, int ySrc, int nSrcWidth, int nSrcHeight, DWORD dwRop) {
  if (g_Shell > WinME ) {
    return pDC->StretchBlt(x, y, nWidth, nHeight, pSrcDC, xSrc, ySrc, nSrcWidth, nSrcHeight, dwRop);
  }
  else {
    for ( int yOffs = 0; yOffs < nHeight;  yOffs += nSrcHeight ) {
      pDC->StretchBlt(x, y+yOffs, nWidth, nSrcHeight, pSrcDC, xSrc, ySrc, nSrcWidth, nSrcHeight, SRCCOPY);
    }
    return true;
  }
}
\************************************** : by VK at 06.01.2007 11:35:46 **/
void GradientEraseBkgnd(CWnd* pWnd, CWnd* pParent, CDC* pDC, CRect &rect, bool fCalcOffs) {
  CRect r;
  ...
  CBitmap* pOldBitmap = dc.SelectObject(&g_GradientBitmap);

  int xOffs  = r.left;
  int xWidth = rect.Width();
/* !!!
  if ( xWidth > g_GradientBitmapWidth ) {
    CreateBackgroundBitmap();
  }
*/
  if ( fCalcOffs ) {
    if ( rect.Width() < g_GradientBitmapWidth ) {
      xOffs = (g_GradientBitmapWidth - rect.Width())/2;
      xWidth -= xOffs;
    }
  }

  // MyStretchBlt(pDC, rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  pDC->StretchBlt(rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  dc.SelectObject(pOldBitmap);
}
void GradientEraseBkgnd(CDC* pDC, CRect &rect, bool fCalcOffs) {
  CDC dc;
  dc.CreateCompatibleDC(pDC);
  CBitmap* pOldBitmap = dc.SelectObject(&g_GradientBitmap);
  int xOffs  = rect.left;
...
/************************************** : by VK at 05.01.2007 4:28:58 **\
  if ( xWidth > g_GradientBitmapWidth ) {
    CreateBackgroundBitmap();
  }
\************************************** : by VK at 05.01.2007 4:29:01 **/
  if ( fCalcOffs ) {
    if ( rect.Width() < g_GradientBitmapWidth ) {
      xOffs = (g_GradientBitmapWidth - rect.Width())/2;
      xWidth -= xOffs;
...
// : by VK at 04.01.2007 23:45:14
//   for ( int y = rect.top; y < rect.Height(); y+= g_GradientBitmapHeight ) {
// 	  pDC->StretchBlt(rect.left, y, rect.Width(), g_GradientBitmapHeight/*rect.Height()*/, &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
//   }
// : by VK at 04.01.2007 23:45:17

  //MyStretchBlt(pDC, rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  pDC->StretchBlt(rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  dc.SelectObject(pOldBitmap);
}
void EraseBkgndForDlgControl(CWnd* pWnd, CDC* pDC, CRect &rect, bool fFrameBG/*=false*/) {
  CRect rParent, r;
  pWnd->GetParent()->GetClientRect(&rParent);
  pWnd->GetWindowRect(&r);
  pWnd->GetParent()->ScreenToClient(&r);
  ...
  int xOffs = r.left;
  int xWidth = rect.Width();
/************************************** : by VK at 05.01.2007 4:27:19 **\
  if ( xWidth > g_GradientBitmapWidth ) {
    CreateBackgroundBitmap();
  }
\************************************** : by VK at 05.01.2007 4:27:21 **/
  if ( fFrameBG ) {
    if ( g_GradientBitmapWidth < rParent.Width() ) {
      xOffs += (g_GradientBitmapWidth - rParent.Width())/2;
      //xWidth -= xOffs;
    }
  }
  else {
/************************************** : by VK at 18.06.2006 9:49:32 **\
    if ( g_GradientBitmapWidth >= rParent.Width() ) {
      xOffs += (g_GradientBitmapWidth - rParent.Width())/2;
      //xWidth -= xOffs;
    }
\************************************** : by VK at 18.06.2006 9:49:36 **/
  }
  //MyStretchBlt(pDC, rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  pDC->StretchBlt(rect.left, rect.top, rect.Width(), rect.Height(), &dc, xOffs, 0, xWidth, g_GradientBitmapHeight, SRCCOPY);
  dc.SelectObject(pOldBitmap);
...
void CNewMenuIcons::SetResourceName(LPCTSTR lpszResourceName)
{
  ...
  {
    int buf_size = _tcslen(lpszResourceName)+sizeof(TCHAR);
    m_lpszResourceName = new TCHAR[buf_size];
#ifdef _STRCPY_S
    _tcscpy_s((LPTSTR)m_lpszResourceName, buf_size, lpszResourceName);
#else
...
{
  switch(GetMenuDrawMode()) {
    case STYLE_MONOCHROME:
    case STYLE_MONOCHROME_NOBORDER:
      return g_clrLeft;
  }
  if(hMenu!=NULL)
  {
...
void CNewMenu::DrawItem_XP_2003(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  COLORREF colorWindow = DarkenColor(10,GetSysColor(COLOR_WINDOW));
/************************************** : by VK 14.12.2004 at 15:40:21 **\
  switch(GetMenuDrawMode()) {
    case STYLE_MONOCHROME:
    case STYLE_MONOCHROME_NOBORDER:
      colorWindow = LightenColor(10,g_clrLeft);
    default:
      colorWindow = DarkenColor(10,g_clrRight);
  }
\************************************** : by VK 14.12.2004 at 15:40:25 **/
  ...
  if(bIsMenuBar)
  {
    ...
    {
      GradientEraseBkgnd(pDC, RectText, false);
/************************************** : by VK at 23.05.2006 3:34:47 **\
      MENUINFO menuInfo = {0};
      menuInfo.cbSize = sizeof(menuInfo);
      menuInfo.fMask = MIM_BACKGROUND;
...
// by VK at 23.05.2006 3:33:14
//        VERIFY(pBrush->UnrealizeObject());
// : by VK at 23.05.2006 3:33:18
...
#ifdef _STRCPY_S
  _tcscpy_s(logfont.lfFaceName, sizeof(logfont.lfFaceName), _T("Marlett"));
#else
...
//    }
  ...
  AddTheme(new CMenuThemeXP(CNewMenu::STYLE_MONOCHROME,
    &CNewMenu::MeasureItem_WinXP,
    &CNewMenu::DrawItem_XP_2003,
    &CNewMenu::DrawMenuTitle,TRUE));

  AddTheme(new CMenuThemeXP(CNewMenu::STYLE_MONOCHROME_NOBORDER,
    &CNewMenu::MeasureItem_WinXP,
    &CNewMenu::DrawItem_XP_2003,
    &CNewMenu::DrawMenuTitle));
...
void CNewMultiDocTemplate::LoadDocString(UINT nIDResource) {
...
/************************************** : by VK at 16.11.2005 7:51:33 **\
  CBrush *pBrush = GetMenuBarBrush();
  if(pBrush)
  {
\************************************** : by VK at 16.11.2005 7:51:36 **/
    CRect rect;
    pDC->GetClipBox(rect);
    GradientEraseBkgnd(this, GetParent(), pDC, rect);
/************************************** : by VK at 06.11.2005 3:57:25 **\
    CRect rectA;
    CRect rectB;
    GetWindowRect(rectA);
    ...
    pDC->SetBrushOrg(oldOrg);
\************************************** : by VK at 06.11.2005 3:57:30 **/
    return true;
/************************************** : by VK at 16.11.2005 7:51:43 **\
  }
  else
  {
    // do default drawing
