...
void GradientEraseBkgnd(CWnd* pWnd, CWnd* pParent, CDC* pDC, CRect &rect);
...
void CButtonST::OnEnable(BOOL bEnable) 
{
	...
	if (bEnable == FALSE)	
	{
    if ( GetFocus()==this ) { // VK: otherwise this code steals current focus
...
BOOL CButtonST::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
  ...
  if ( HTCLIENT==nHitTest ) {
    AfxGetMainWnd()->SendMessage(WM_SETMESSAGESTRING, GetDlgCtrlID()); 
  }
...
/************************************** : by VK at 23.05.2006 3:50:20 **\
		::FillRect(hDC, &rRect, (HBRUSH)RGB(255, 255, 255));
\************************************** : by VK at 23.05.2006 3:50:22 **/
		COLORREF crOldColor = ::SetBkColor(hDC, RGB(255,255,255));
    CDC::FromHandle(hDC)->FillSolidRect(&rRect, RGB(255, 255, 255));
