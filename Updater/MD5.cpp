...
void MD5Final (char* cReturnStr) {
  ...
  for (i=0; i < nMD5Size; i++) {
    ...
    if (lpszMD5[i] == 0) {
      ...
      sprintf_s(temp, sizeof(temp), "0%X",lpszMD5[i]);
    }
    else {
      sprintf_s(temp, sizeof(temp), "%X",lpszMD5[i]);
    }
...
void GetMD5(unsigned char* pBuf, UINT nLength,char* cReturnStr){
  MD5Init();
...
BOOL CalcMD5(LPCTSTR szFilename, CString& szMD5) {
  CFile f;
  if ( f.Open(szFilename, CFile::modeRead | CFile::shareDenyWrite) ) {
    ...
    if ( cbSize > 0 ) {
      ...
      if ( pBuf ) {
        UINT cbRead = f.Read(pBuf, cbSize);
        if ( cbSize==cbRead ) {
          GetMD5(pBuf, cbSize, md5str);
        }
        delete pBuf;
        szMD5 = md5str;
      	return (cbSize==cbRead);
      }
    }
  }
  return false;
