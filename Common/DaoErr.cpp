...
CString ExplanationFieldName(CDaoException* e, CDaoRecordset* prs=NULL, int nField=-1) {
  CString s;
...
void  DaoError(CDaoException* e, LPCTSTR szFieldName/*=NULL*/, CDaoRecordset* prs/*=NULL*/, int nField/*=-1*/) {
  ...
  switch(e->m_pErrorInfo->m_lErrorCode) {
    ...
    case 3163:  //	The field is too small to accept the amount of data you attempted to add. Try inserting or pasting less data.
      if ( szFieldName ) {
        sErr.Format(_T("Поле \"%s\" не вміщую таку кілкість даних"), szFieldName);
      }
      else if ( prs != NULL && nField != -1 ) {
        sErr.Format(_T("Поле \"%s\" не вміщую таку кілкість даних"), ExplanationFieldName(e, prs, nField) );
      }
      break;
...
CString FormatDaoError(CDaoException* e) {
  CString s;
  s.Format(_T("Помилка DAO %d: %s"), e->m_pErrorInfo->m_lErrorCode, e->m_pErrorInfo->m_strDescription);
  return s;
}